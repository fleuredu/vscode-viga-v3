...
Usage:
    python git_push.py --message "Auto-update" --branch main
"""
...
def git_commit(path: str, message: str, author: str = None) -> bool:
    ...
    if proc.returncode == 0:
        print(f"[AUTOGIT] Committed: {message}")
        return True
    else:
        ...
        if "nothing to commit" in out or "no changes added to commit" in out:
            ...
            print(f"[AUTOGIT] Commit failed: {proc.stderr.strip()}")
            raise subprocess.CalledProcessError(proc.returncode, cmd, output=proc.stdout, stderr=proc.stderr)
...
def git_push(path: str, remote: str, branch: str, max_retries: int = 3, retry_delay: int = 3) -> None:
    ...
    while True:
        attempt += 1
        print(f"[AUTOGIT] Pushing to {remote}/{branch} (attempt {attempt})")
        proc = run_cmd(["git", "push", remote, branch], cwd=path)
        if proc.returncode == 0:
            print("[AUTOGIT] Push successful.")
            return
        print(f"[AUTOGIT] Push failed (exit {proc.returncode}). stderr={proc.stderr.strip()}")
        if attempt >= max_retries:
            print(f"[AUTOGIT] Push failed after {attempt} attempts.")
...
def main():
    ...
    print(f"[AUTOGIT] Repo path: {repo_path}")

    if not is_git_repo(repo_path):
        print(f"[AUTOGIT] Not a git repository: {repo_path}")
        sys.exit(2)
    ...
    try:
        ...
        if not has_changes(repo_path):
            print("[AUTOGIT] No changes detected. Nothing to do.")
            print("[AUTOGIT] No changes to commit.")
            return
        ...
        if not args.no_push:
            ...
            print("[AUTOGIT] Skipping push (--no-push set)")
